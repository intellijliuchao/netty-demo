...
public class TimeClientHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(final ChannelHandlerContext ctx) {
        /*
         * Scanner cin=new Scanner(System.in); System.out.println("请输入发送信息：");
         ...
         * 当你向ByteBuf里写入数据的时候写指针的索引就会增加， 同时读指针的索引没有变化。 读指针索引和写指针索引分别代表了消息的开始和结束。
         * 比较起来，NIO缓冲并没有提供一种简洁的方式来计算出消息内容的开始和结尾， 除非你调用flip方法。
         * 当你忘记调用flip方法而引起没有数据或者错误数据被发送时， 你会陷入困境。这样的一个错误不会发生在Netty上，
         ...
         * 因为在Netty里所有的操作都是异步的。 因此你需要在write()方法返回的ChannelFuture完成后调用close()方法，
